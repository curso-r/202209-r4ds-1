...
params:
  ...
  turma: 202209-r4ds-1
  nome_curso: R para Ciência de Dados I
  num_aula: '7'
...
knitr::kable(
  tibble::tibble(
    script = CursoRopen:::list_github_files(params$main_repo, pasta, ".R$|.Rmd$"),
    link = paste0(
...
knitr::kable(
  tibble::tibble(
    script = list.files( pasta, ".R$|.qmd$|.Rmd$"),
    link = paste0(
      "https://github.com/curso-r/",
      params$turma,
      "/blob/main/",
...
tarefas_raw <-
  ...
  googlesheets4::read_sheet("licao-de-casa-r4ds-1") 

tarefas <- tarefas_raw %>%
  dplyr::mutate(link = glue::glue("[{desc}]({url})"),
                respostas = glue::glue("[Respostas]({url_respostas})")) %>%
  dplyr::arrange(aula) %>%
  dplyr::select(-url_respostas, -url, -desc) |> 
...
```{r}
tarefas |> 
  dplyr::filter(tipo_tarefa == "tarefa") |> 
  dplyr::select(-tipo_tarefa) |> 
  knitr::kable(col.names = c("Aula", "Descrição", "Respostas"))
```
...
```{r}
tarefas |> 
  dplyr::filter(tipo_tarefa == "leitura") |> 
  dplyr::select(-tipo_tarefa, -respostas) |> 
...
```{r}
tarefas |> 
  dplyr::filter(tipo_tarefa == "tarefa complementar") |> 
  dplyr::select(-tipo_tarefa, -respostas) |> 
  knitr::kable(col.names = c("Aula", "Descrição"))
```
